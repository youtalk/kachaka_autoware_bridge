...
def travel_direction_from_tangent(angular_position: float, tangent_yaw: float) -> str:
    """Classify a loop lanelet's travel direction from its centerline tangent.

    ``angular_position`` = atan2(point_y - center_y, point_x - center_x) for a
    point on the loop; ``tangent_yaw`` = heading of the lanelet centerline there.
    Counter-clockwise travel has tangent ~ angular_position + pi/2, clockwise
    ~ angular_position - pi/2. Used by set_loop_route to read the *actual* travel
    ...
    """
    d_ccw = abs(_normalize_angle(tangent_yaw - _tangent_yaw(angular_position, 1.0)))
    d_cw = abs(_normalize_angle(tangent_yaw - _tangent_yaw(angular_position, -1.0)))

...
VehicleInterfaceNode::VehicleInterfaceNode(const rclcpp::NodeOptions& options)
  ...
  params.max_linear_velocity =
      declare_parameter<double>("max_linear_velocity", 0.3);
  params.max_angular_velocity =
      declare_parameter<double>("max_angular_velocity", 1.57);
  ...
  const bool auto_enable =
      declare_parameter<bool>("auto_enable_manual_control", true);
  ...
  control_sub_ = create_subscription<autoware_control_msgs::msg::Control>(
      ...
      std::bind(&VehicleInterfaceNode::on_control, this,
                std::placeholders::_1));
  ...
  enable_manual_control_client_ = create_client<std_srvs::srv::SetBool>(
  ...
  RCLCPP_INFO(get_logger(),
              "VehicleInterfaceNode started: wheel_base=%.3f, vmax=%.3f, "
              "wmax=%.3f, timeout=%.2fs",
              params.wheel_base, params.max_linear_velocity,
              params.max_angular_velocity, cmd_vel_timeout_sec_);
}
...
void VehicleInterfaceNode::on_odom(
...
void VehicleInterfaceNode::on_velocity_status_timer() {
  ...
  velocity_status_pub_->publish(
      convert_odometry_to_velocity_report(*latest_odom_));
}
...
void VehicleInterfaceNode::on_op_mode_timer() {
  ...
  state.is_autoware_control_enabled =
      (state.mode == OperationModeStateMsg::AUTONOMOUS);
...
void VehicleInterfaceNode::on_cmd_vel_timeout_timer() {
  ...
  // Skip until the first control message has arrived (last_control_stamp_ ==
  // 0).
...
void VehicleInterfaceNode::on_change_to_autonomous(
    const autoware_adapi_v1_msgs::srv::ChangeOperationMode::Request::
        SharedPtr /*req*/,
...
void VehicleInterfaceNode::enable_manual_control(bool enable) {
  ...
  enable_manual_control_timer_ = create_wall_timer(500ms, [this]() {
    ...
    RCLCPP_INFO(get_logger(), "Requested set_manual_control_enabled(%s)",
